---
title: "Evaluation report for `r params$code`"
author: "medfateTeam"
date: "`r Sys.time()`"
params:
...
if(control=="") control = medfate::defaultControl()
...
if (params$transpMode %in% c('simple', 'both')) {
  ...
  # model run
  res_simple <- medfate::spwb(simple_input, soil_object, meteoData)
} else {
...
if (params$transpMode %in% c('both')) {
  ...
  if (params$transpMode %in% c('simple')) {
    sp_names <- paste0('E_sp', simple_input$above$SP)
    models_dfs <- saveRes(res_simple, NULL, e_measured_coh,
                          spParams = sp_params,
                          site_code = code)
  } else {
    sp_names <- paste0('E_sp', complex_input$above$SP)
    models_dfs <- saveRes(NULL, complex_res = res_complex, e_measured_coh,
                          spParams = sp_params, site_code = code)
  }
}
```
...
Comparison | MAE | r² | Bias
------------|-----|----|-------
Simple *vs.* Measured | `r Eplanttot_stats$MAE_simple` | `r Eplanttot_stats$r_sq_simple` | `r Eplanttot_stats$bias_simple`
Complex *vs.* Measured | `r Eplanttot_stats$MAE_complex` | `r Eplanttot_stats$r_sq_complex` | `r Eplanttot_stats$bias_complex`
Simple *vs.* Complex | `r Eplanttot_stats$MAE_both` | `r Eplanttot_stats$r_sq_both` | `r Eplanttot_stats$bias_both`
